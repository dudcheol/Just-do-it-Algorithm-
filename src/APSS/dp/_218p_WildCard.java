...
/*
 * 알고리즘 문제해결 전략 - 218p - 와일드카드 (완전탐색ver)
 * - 교재참고! -
 *	[간단한 해법]
 ...
 *	[해법을 찾는 데 결정적인 깨달음]
	'*'을 기준으로 패턴을 나누는 것 자체를 생각하지 못했었다. 또, 재귀호출의 응용방법이 배웠던 부분에서만 생각이 가능했는데
	이런식으로도 가능하다는 것을 깨달았다.
 *	[다른 해결 방법이 있다면?]
...
public class _218p_WildCard {
	...
	static boolean findMatch(String w, String s) {
		/*
		...
		if (w.charAt(pos) == '*') {
			for (int skip = 0; pos + skip <= s.length(); skip++) {
				if (findMatch(w.substring(pos + 1), s.substring(pos + skip))) {
					return true;
				}
			}
		}
