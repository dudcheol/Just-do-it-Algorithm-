...
public class Start extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String query = "select num, name from customer order by num"; // + asc:오름차순, + desc:내림차순
			...
			while (rs.next()) { // rs.next() : 실제 데이터를 호출하기 위해선 반드시 호출할 것!
				...
				out.println("<td><a href=Detail?num=" + num + ">" + name + "</a></td>");
				out.println("</tr>");
			}
			out.println("</table>");
			out.println("<a href=form/insert.html>새 고객 등록</a>"); // Java가 아니면 확장자를 모두 명시해주어야 함
